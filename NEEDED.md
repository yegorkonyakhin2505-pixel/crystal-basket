...
| **N18** 🚨 | Shopify stock storefront: `noindex` + path-preserving redirect | Since 2026-09-19 the Shopify storefront answers on `shop.crystalbasket.store` (primary domain, so checkout is branded and analytics share one root domain). Until the line is pasted, that subdomain shows the stock Shopify theme and is indexable | Replace line 9 of `layout/theme.liquid` with the line in `docs/shopify/redirect-snippet.liquid` (tested: products keep their handle, collections → /shop/, policies → our policy pages; checkout, `/cart/c/` handoff, accounts and order pages untouched). Shopify's new code editor ignores automated input (2026-09-15), so this needs a 2-minute paste or approval to sign in the Shopify CLI | ⏳ |
| **N19** | ~~Founder name and photo~~ | Decided 2026-09-15: the brand stays discreet, no founder name or photo. Trust comes from policies, process copy, real reviews (N03) and real product photos (N05) | | ✅ decided |
