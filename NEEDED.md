...
| **N18** | Shopify stock storefront: `noindex` + path-preserving redirect | `utx8rj-t3.myshopify.com` pages redirect to the homepage instead of the matching page, and are indexable duplicates | Replace line 9 of `layout/theme.liquid` with the line in `docs/shopify/redirect-snippet.liquid` (tested: products keep their handle, collections → /shop/, policies → our policy pages; checkout, `/cart/c/` handoff, accounts and order pages untouched). Shopify's new code editor ignores automated input (2026-09-15), so this needs a 2-minute paste or approval to sign in the Shopify CLI | ⏳ |
