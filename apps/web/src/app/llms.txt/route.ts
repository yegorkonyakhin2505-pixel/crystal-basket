...
export function GET() {
  ...
  const lines: string[] = [
    ...
    "## Facts",
    "",
    `- Brand: ${site.name}, ${site.city}, United Arab Emirates. Instagram: https://instagram.com/${site.instagram}. Email: ${site.email}.`,
    `- Every bracelet is ${BEAD_MM} mm beads, wrist sizes S 16 cm, M 18 cm, L 20 cm; custom lengths on request.`,
    `- Prices ${Math.min(...products.map((p) => p.data.priceAED))} to ${Math.max(...products.map((p) => p.data.priceAED))} AED. Delivery 25 AED, free over ${site.freeDeliveryAED} AED, 1 to 2 business days in the UAE.`,
    ...
    "## Stacks",
    "",
    ...getStacks().map((s) => `- [${s.data.name}](${u(routes.stacks)}): ${s.data.description} ${s.data.priceAED} AED.`),
    "",
    "## Guides",
    "",
    ...
    `- [Our story](${u(routes.about)}): who makes the bracelets and how.`,
