...
export function WishlistClient({ items }: { items: Item[] }) {
  ...
  const waUrl = useMemo(() => orderByMessageUrl(saved.map((s) => ({ name: s.name, priceAED: s.priceAED }))), [saved]);
  ...
  return (
    <>
      ...
      <div className="mt-8 flex flex-wrap gap-3">{waUrl && <a href={waUrl} target="_blank" rel="noopener" className="inline-flex h-12 px-8 items-center bg-cb-ink text-white text-[12px] uppercase tracking-[0.14em] hover:bg-black">Order all on WhatsApp</a>}<ButtonLink href={routes.shop} variant="outline" size="lg">Keep browsing</ButtonLink></div>
    </>
  );
}
