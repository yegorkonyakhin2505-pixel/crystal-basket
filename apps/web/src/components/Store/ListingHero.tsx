...
export function ListingHero({ image, alt = "", crumbs, title, text, palette }: { image?: string | null; alt?: string; crumbs: [string, string][]; title: string; text?: string; palette?: [string, string] }) {
  return (
    <section className="relative min-h-[300px] md:min-h-[380px] overflow-hidden bg-cb-band">
      {image ? (
        <div className="absolute inset-0"><Img src={asset(image)} alt={alt} sizes="100vw" loading="eager" className="object-[70%_center]" /></div>
      ) : palette ? (
