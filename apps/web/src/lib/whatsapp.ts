...
/** `totalAED` overrides the computed sum when a discount applies (stacks). */
export function buildWhatsAppUrl(lines: OrderLine[], note?: string, totalAED?: number): string {
  const body = lines
    .map((l) => {
    ...
    .join("\n");
  const total = totalAED ?? lines.reduce((s, l) => s + l.priceAED * (l.qty ?? 1), 0);
