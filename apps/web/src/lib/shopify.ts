...
async function gql<T>(query: string, variables: Record<string, unknown> = {}): Promise<T> {
  ...
  if (!res.ok || json.errors?.length) throw new Error(json.errors?.map((e) => e.message).join("; ") ?? `Shopify ${res.status}`);
...
const CART_FIELDS = `
  ...
  lines(first: 50) { nodes { id quantity merchandise { ... on ProductVariant { title product { title handle } price { amount } image { url } } } } }
`;
interface RawCart { id: string; checkoutUrl: string; totalQuantity: number; cost: { subtotalAmount: { amount: string } }; lines: { nodes: { id: string; quantity: number; merchandise: { title: string; product: { title: string; handle: string }; price: { amount: string }; image?: { url: string } } }[] } }
...
/** Resolve the Shopify variant for a product handle + bead size + wrist size (option names match the CSV import). */
export async function findVariantId(handle: string, beadMm: number, size: string): Promise<string> {
  ...
  const v = data.product?.variants.nodes.find((n) => {
    const bead = n.selectedOptions.find((o) => o.name.toLowerCase().startsWith("bead"))?.value ?? "";
    const wrist = n.selectedOptions.find((o) => o.name.toLowerCase().startsWith("wrist"))?.value ?? "";
    return bead.startsWith(String(beadMm)) && wrist.trim().toUpperCase().startsWith(size.toUpperCase());
  });
  if (!v) throw new Error(`No variant for ${handle} ${beadMm}mm ${size}. Import the product CSV first.`);
  return v.id;
}

export async function cartCreate(lines: { merchandiseId: string; quantity: number }[]): Promise<Cart> {
  const d = await gql<{ cartCreate: { cart: RawCart } }>(`mutation($lines: [CartLineInput!]!) { cartCreate(input: { lines: $lines }) { cart { ${CART_FIELDS} } } }`, { lines });
...
export async function cartLinesAdd(cartId: string, lines: { merchandiseId: string; quantity: number }[]): Promise<Cart> {
  const d = await gql<{ cartLinesAdd: { cart: RawCart } }>(`mutation($cartId: ID!, $lines: [CartLineInput!]!) { cartLinesAdd(cartId: $cartId, lines: $lines) { cart { ${CART_FIELDS} } } }`, { cartId, lines });
  return shape(d.cartLinesAdd.cart);
}
export async function cartLinesRemove(cartId: string, lineIds: string[]): Promise<Cart> {
  const d = await gql<{ cartLinesRemove: { cart: RawCart } }>(`mutation($cartId: ID!, $lineIds: [ID!]!) { cartLinesRemove(cartId: $cartId, lineIds: $lineIds) { cart { ${CART_FIELDS} } } }`, { cartId, lineIds });
  return shape(d.cartLinesRemove.cart);
}
