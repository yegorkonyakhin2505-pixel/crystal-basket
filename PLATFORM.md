...
| `/build/` | `build/page.tsx` | stones, intentions, `site.custom` | Build-your-own bracelet: `BraceletBuilder` (client). 2.5D SVG ring: beads are photographic sphere sprites (`public/images/beads/<id>.png`, alpha PNGs cut from one Nano Banana Pro 6×3 grid; gradient fallback) with rim and specular overlays, front beads larger and drawn last, positions animate on reflow. Wrist size sets the 8 mm bead count (S 20, M 23, L 25); tap a stone to drop a bead (`beadDrop` + landing `beadRipple`), slider drops several, repeat pattern, fill the rest, undo, tap a bead to pop it out (`beadPop`); floating ring, pulsing next slot, completion settle + sparkles, ticking price, styled slider. Stone chips show the tumbled-stone photos. Price = `site.custom.baseAED` + highest stone tier + gold bead. Complete designs resolve the Shopify `custom-bracelet` variant (Wrist size × Stones tier × Gold bead) via `findVariantByOptions` and add it with the bead sequence as line attributes; `?d=<size>.<idx>...` share links. Email fallback when Shopify is off |
