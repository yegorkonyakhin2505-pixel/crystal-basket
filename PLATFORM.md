...
| `/build/` | `build/page.tsx` | stones, intentions, `site.custom` | Build-your-own bracelet: `BraceletBuilder` (client). 2.5D SVG ring with the active slot always at the front: beads are photographic sphere sprites (`public/images/beads/<id>.png`, alpha PNGs cut from one Nano Banana Pro 6×3 grid; gradient fallback) with rim and specular overlays, scaled by depth, positions animate on every reflow. Tapping a stone chip strings `qty` beads: each bead flies from the chip along an arc (JS RAF flight into the front slot), lands with a ripple, then the whole ring turns one bead so the next empty slot faces you (`placed` counter drives the rotation, pending beads queue). Wrist size sets the 8 mm bead count (S 20, M 23, L 25); fill the rest, repeat pattern, undo, tap a bead to pop it out and the ring turns back; ghost preview of the selected stone in the front slot, floating ring, completion settle + sparkles, ticking price, styled slider. Stone chips carry `data-chip=<id>` (flight origin). Price = `site.custom.baseAED` + highest stone tier + gold bead. Complete designs resolve the Shopify `custom-bracelet` variant (Wrist size × Stones tier × Gold bead) via `findVariantByOptions` and add it with the bead sequence as line attributes; `?d=<size>.<idx>...` share links. Email fallback when Shopify is off |
