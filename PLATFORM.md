...
| `/build/` | `build/page.tsx` | stones, intentions, `site.custom` | Build-your-own bracelet: `BraceletBuilder` (client). Tray of photographic bead sprites (`public/images/beads/<id>.png`, feathered alpha PNGs cut from one Nano Banana Pro 6×3 grid) above `BraceletStage`, a canvas engine that draws every frame (RAF while anything moves, idle otherwise; DPR-aware; reduced-motion aware). Tapping a tray bead (`data-tray=<id>`) adds `qty` beads; each falls from the tray on a gravity arc with a squash on landing, taps queue 120 ms apart, beads slide along the string on a damped spring so the row stays centred and touching, and the string sags/bounces under the weight. When every slot is filled (S 20, M 23, L 25 beads of 8 mm), **Done · form the bracelet** morphs the string into a tilted 3D circle with perspective (per-point curl from the ends inward, beads ride the string and grow with the view; front beads larger, back smaller, all touching in world space), then a glint sweeps across; Edit beads unfolds. Tap a bead to pop it off (canvas hit-test), hover lifts it; undo, clear, fill with the last stone, repeat the pattern, beads-per-tap slider, ticking price. No placeholder rings, slot circles or landing rings. Price = `site.custom.baseAED` + highest stone tier + gold bead. Complete designs resolve the Shopify `custom-bracelet` variant (Wrist size × Stones tier × Gold bead) via `findVariantByOptions` and add it with the bead sequence as line attributes; `?d=<size>.<idx>...` share links. Email fallback when Shopify is off |
