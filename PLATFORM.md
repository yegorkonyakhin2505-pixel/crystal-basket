...
| `/build/` | `build/page.tsx` | stones, intentions, `site.custom` | Build-your-own bracelet: `BraceletBuilder` (client). Tray of photographic bead sprites (`public/images/beads/<id>.png`, feathered alpha PNGs cut from one Nano Banana Pro 6×3 grid) above a loose string drawn in SVG. Tapping a tray bead (`data-tray=<id>`) drops `qty` beads: each falls from the tray along an arc (RAF flight with squash on landing + ripple) onto the string, where beads sit touching in a centred row (arc-length sampling of the cord curve); taps queue while stringing. When every slot is filled (S 20, M 23, L 25 beads of 8 mm), **Done · form the bracelet** curls the string into an ellipse seen from a raised angle (each bead glides to its ring position with a stagger, depth scaling, sparkles); Edit beads unfolds it. Tap a bead on the string to pop it off; undo, clear, fill with the last stone, repeat the pattern, beads-per-tap slider, ticking price. No placeholder rings or slot circles are drawn. Price = `site.custom.baseAED` + highest stone tier + gold bead. Complete designs resolve the Shopify `custom-bracelet` variant (Wrist size × Stones tier × Gold bead) via `findVariantByOptions` and add it with the bead sequence as line attributes; `?d=<size>.<idx>...` share links. Email fallback when Shopify is off |
